  ...
  return (
    <div className="relative py-10">
      ...
      <div className="absolute w-9/12 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center text-white flex flex-col gap-y-5 md:gap-y-44">
        ...
        <h1
          className={`${fraunces.className} md:text-5xl md:leading-tight font-light capitalize`}
        >
          {singlePost.title}
        </h1>
        <Link href="/" as={dynamicUrl}>
          {" "}
          <button className="w-3/12 md:w-[15%] mx-auto border border-white rounded-full text-xs md:text-sm font-semibold py-2 hover:text-black hover:bg-white transition-all ease-linear">
            {" "}
            Read Now{" "}
          </button>{" "}
        </Link>
