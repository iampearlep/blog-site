...
const Feature = async() => {
  ...
  return (
   <div className='relative py-10'>
    ...
    <Link href={`/article/${singlePost.slug}`}> <button className='w-3/12 md:w-[15%] mx-auto border border-white rounded-full text-xs md:text-sm font-semibold py-2 hover:text-black hover:bg-white transition-all ease-linear'> Read Now </button> </Link>
    </div>
   </div>
  )
}
